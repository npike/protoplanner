class ComponentManager {
    ...
    get componentLayer() {
        return this.board.componentLayer;
    }

    addComponent(type, x, y) {
        const def = ComponentRegistry.get(type);
        if (!def) return;
        const h = this.board.getNearestHole(x, y);
        if (h) this.createComponent(def, h);
    }

    createComponent(def, anchorHole) {
        const mountedSide = this.board.side;
        const initialPitch = def.pins.length > 1 ? (def.pins[1].dy - def.pins[0].dy) : 0;
        
        const pinIds = this.calculatePinIds(def, anchorHole, 0, initialPitch, mountedSide);
        if (!pinIds) return console.warn("Doesn't fit");

        const compId = `comp_${Date.now()}`;
        this.components.push({ 
            id: compId, type: def.type, pinIds, element: Utils.createSVGElement('g', { class:'component', cursor:'grab' }), 
            rotation: 0, pitch: initialPitch, anchorId: anchorHole.id, mountedSide, locked: false,
            customLabel: "" 
        });
        
        this.components[this.components.length-1].element.setAttribute('data-id', compId);
        this.renderAll(this.board.side);
    }

    calculatePinIds(def, anchorHole, rotation, pitch, side) {
        const pinIds = [];
        ...
        for (let i = 0; i < def.pins.length; i++) {
            ...
            const h = this.board.getHoleByLogical(lxAnchor + rot.x, lyAnchor + rot.y);
            if (!h) return null;
            pinIds.push(h.id);
        }
        return pinIds;
    ...
    renderComponent(comp) {
        ...
        const g = comp.element; g.innerHTML = '';
        const vPos = this.board.getHoleCoords(comp.anchorId, comp.mountedSide);
        
        const bX = vPos.cx + (def.body.offsetX * this.board.pitch);
        const bY = vPos.cy + (def.body.offsetY * this.board.pitch);
        
        g.appendChild(Utils.createSVGElement('rect', {
            x: bX, y: bY, width: def.body.width * this.board.pitch, height: def.body.height * this.board.pitch,
        ...
        def.visuals.forEach(vis => {
            if (vis.shape === 'rect') {
                ...
                g.appendChild(Utils.createSVGElement('rect', {
                    x: vPos.cx + (vx * this.board.pitch), y: vPos.cy + (vis.y * this.board.pitch),
                    width: vis.w * this.board.pitch, height: vis.h * this.board.pitch, fill: vis.fill, rx: vis.rx || 0
                }));
            } else if (vis.shape === 'circle') {
                const cx = vPos.cx + ((comp.mountedSide === 'top' ? vis.cx : -vis.cx) * this.board.pitch);
                g.appendChild(Utils.createSVGElement('circle', { cx, cy: vPos.cy + (vis.cy * this.board.pitch), r: vis.variant === 'screw' ? 5 : (vis.r * this.board.pitch), fill: vis.fill, stroke: vis.stroke }));
                ...
                g.appendChild(Utils.createSVGElement('line', {
                    x1: vPos.cx + (vx1 * this.board.pitch), y1: vPos.cy + (vis.y1 * this.board.pitch),
                    x2: vPos.cx + (vx2 * this.board.pitch), y2: vPos.cy + (vy2 * this.board.pitch),
        ...
        def.pins.forEach((off, i) => {
            ...
            const cx = vPos.cx + ((comp.mountedSide === 'top' ? off.dx : -off.dx) * this.board.pitch);
            const cy = vPos.cy + (dy * this.board.pitch);
        ...
        if (comp.customLabel) {
            const labelY = bY - 5, labelX = bX + (def.body.width * this.board.pitch / 2);
    ...
    renderAll(side) {
        ...
        this.components.forEach(comp => {
            if (comp.mountedSide === side) {
                this.renderComponent(comp); this.componentLayer.appendChild(comp.element);
            } else {
                const g = Utils.createSVGElement('g', { class: 'component-pins', 'data-id': comp.id });
                const def = ComponentRegistry.get(comp.type);
                comp.pinIds.forEach((id, i) => {
                    const h = this.board.getHoleCoords(id, side);
                    if (h) {
    ...
    renderGhost(type, anchorHole, rotation = 0, pitch = null) {
        this.clearGhost();
        const def = ComponentRegistry.get(type);
        if (!def || !anchorHole) return;

        const effectivePitch = pitch !== null ? pitch : (def.pins.length > 1 ? (def.pins[1].dy - def.pins[0].dy) : 0);
        const pinIds = this.calculatePinIds(def, anchorHole, rotation, effectivePitch, this.board.side);
        const isValid = !!pinIds;

        const g = Utils.createSVGElement('g', { class: 'component-ghost', opacity: 0.6 });
        const vPos = this.board.getHoleCoords(anchorHole.id, this.board.side);
        ...
        g.appendChild(Utils.createSVGElement('rect', {
            x: vPos.cx + (def.body.offsetX * this.board.pitch), 
            y: vPos.cy + (def.body.offsetY * this.board.pitch), 
            width: def.body.width * this.board.pitch, height: def.body.height * this.board.pitch,
            fill: fill, stroke: stroke, rx: def.body.rx || 0
        }));

        const vRot = this.board.side === 'top' ? rotation : -rotation;
        g.setAttribute('transform', `rotate(${vRot}, ${vPos.cx}, ${vPos.cy})`);
        this.board.ghostLayer.appendChild(g);
    }

    clearGhost() {
        this.board.ghostLayer.innerHTML = '';
    }

    moveComponent(id, anchor) {
        const comp = this.getComponentById(id);
        if (!comp || comp.locked) return false;
        const def = ComponentRegistry.get(comp.type);
        const pinIds = this.calculatePinIds(def, anchor, comp.rotation, comp.pitch, this.board.side);
    ...
    rotateComponent(id) {
        ...
        const def = ComponentRegistry.get(comp.type);
        const anchor = this.board.getHoleById(comp.anchorId);
        const nextRot = (comp.rotation + 90) % 360;
        const pinIds = this.calculatePinIds(def, anchor, nextRot, comp.pitch, this.board.side);
        if (!pinIds) return;
    ...
    adjustPitch(id, delta) {
        ...
        const anchor = this.board.getHoleById(comp.anchorId);
        const pinIds = this.calculatePinIds(def, anchor, comp.rotation, nextP, this.board.side);
        if (!pinIds) return;
    ...
    setCustomLabel(id, label) {
        const comp = this.getComponentById(id);
        if (comp) { comp.customLabel = label; this.renderComponent(comp); }
    }
    ...
    deserialize(data) {
        ...
        data.forEach(d => {
            ...
            const anchor = this.board.getHoleById(d.a);
            if (def && anchor) {
                const pinIds = this.calculatePinIds(def, anchor, d.r, d.p, d.s);
                if (pinIds) {
                    this.components.push({
                        id: `comp_${Date.now()}_${Math.random()}`,
                        type: d.t, pinIds, element: Utils.createSVGElement('g', { class:'component', cursor:'grab' }),
                        rotation: d.r, pitch: d.p, anchorId: d.a, mountedSide: d.s, locked: !!d.k,
                        customLabel: d.l || ""
                    });
                    const comp = this.components[this.components.length-1];
                    comp.element.setAttribute('data-id', comp.id);
                    if (comp.locked) comp.element.classList.add('locked');
                }
            }
        });
        this.renderAll(this.board.side);
    ...
    setSelection(id, isSel) {
        const comp = this.getComponentById(id);
        if (comp) (this.componentLayer.querySelector(`[data-id="${id}"]`) || comp.element).classList.toggle('selected', isSel);
    }
    toggleLock(id) {
        const comp = this.getComponentById(id);
        if (comp) { comp.locked = !comp.locked; (this.componentLayer.querySelector(`[data-id="${id}"]`) || comp.element).classList.toggle('locked', comp.locked); return comp.locked; }
        return false;
    }
    removeComponent(id) {
        const index = this.components.findIndex(c => c.id === id);
        if (index > -1) {
            const el = this.componentLayer.querySelector(`[data-id="${id}"]`);
            if (el) this.componentLayer.removeChild(el);
            this.components.splice(index, 1);
        }
    }
    clear(silent = false) {
        this.components.forEach(c => {
            const el = this.componentLayer.querySelector(`[data-id="${c.id}"]`);
            if (el) this.componentLayer.removeChild(el);
        });
        this.components = [];
    }
